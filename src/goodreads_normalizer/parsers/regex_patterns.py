...
""" This rule handles """
RULE_1 = re.compile(r"^(?P<Title>[\w\s'.,-]+?)(?:$|(?P<HasSeries>\s\()"
...
RULE_2 = re.compile(r"^(?=.*book)(?P<Title>[\w\s'-]+.*)"
...
RULE_4 = re.compile(r"(?P<Title>.*)\s\((?:(?P<ForgottenRealms>Forgotten\sRealms):\s)?"
...
TITLE_ONLY = re.compile(r"^(?!.*[:#()])(?P<Title>.*)$", flags=re.IGNORECASE | re.MULTILINE)